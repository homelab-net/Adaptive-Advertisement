...
## Update — blocker closure (same day)

Follow-up fix applied after this initial report:

- `services/dashboard-api/dashboard_api/audience_sink.py` now imports via absolute package paths (`dashboard_api.*`) to support module-file loading used by runtime lint tests.
- Privacy-violation logging was reduced to a generic warning without flag values/tokens that trigger PII runtime lint patterns.

Post-fix validation in this environment:

- `pytest tests/integration/test_log_pii_lint.py::TestRuntimeLogPIILint::test_audience_sink_privacy_gate_logs_no_pii -q` → pass
- `pytest tests/integration -q` → 70 passed
- `pytest -q` (repo root with service paths set in `PYTHONPATH`) → 391 passed
