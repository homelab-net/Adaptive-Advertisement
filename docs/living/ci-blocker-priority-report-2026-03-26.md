...
2. Keep package specifiers quoted/escaped in shell contexts (`ci.yml` and Dockerfiles) so `>=` is not parsed as shell redirection.
3. Execute the exact GitHub workflow on Python 3.11 runner with unrestricted package access.
4. If compose-smoke remains red, inspect failing service health endpoint logs and pin to service-level defects.
5. Gate merges on required checks listed above.
...
## Cross-environment compatibility updates

To keep the same repository runnable in both constrained local environments and full CI:

- Removed an unused `freezegun` import in `services/input-cv/tests/unit/test_health_tracker.py` that could fail collection when dependency mirrors are restricted.
- Added optional-dependency guards in dashboard-api tests so missing local `httpx` / `aiosqlite` cleanly skip dashboard-api unit tests rather than hard-failing collection.
- CI remains strict: GitHub Actions installs dashboard-api test dependencies, so these tests execute normally there.
